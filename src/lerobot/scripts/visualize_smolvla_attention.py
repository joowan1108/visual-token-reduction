...
def _overlay(image: np.ndarray, attention: np.ndarray, peak: float) -> Image.Image:
    ...
    normalized = attention.reshape(grid_size, grid_size) / peak if peak else np.zeros_like(
        attention.reshape(grid_size, grid_size)
    )
...
def render_attention_maps(input_path: Path, output_dir: Path) -> int:
    ...
    for npz_path in files:
        ...
        for batch_idx in range(images.shape[0]):
            ...
            canvas = Image.new(
                "RGB", (canvas_width, TITLE_HEIGHT + 2 * (height + LABEL_HEIGHT)), "white"
            )
            ...
            for camera_idx in range(images.shape[1]):
                ...
                draw.text(
                    (x + 4, y + 4),
                    f"{kind}/{scope} layers={layers} | flow={flow_step} | "
                    f"mass={masses[batch_idx, camera_idx]:.4f}",
                    fill="black",
                )
